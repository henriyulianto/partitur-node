...
import type { Lagu, WorkInfo, CDN, Files, MusicalStructure, MeasureHighlighters, URLs } from '../src/types/interfaces.js';
 ...
 */
function getRawGithubFileUrl(title: string, filename: string): string {
 ...
 */
function processSongDirectory(songDir: string, baseDataPath: string): Lagu | null {
  try {
 ...
 */
async function generateIndexJson(): Promise<void> {
 ...
 */
async function generateSongDataFiles(): Promise<void> {
  const publicDataPath = path.join(__dirname, '..', 'public', 'partitur-data');
  const distDataPath = path.join(__dirname, '..', 'dist', 'partitur-data');
