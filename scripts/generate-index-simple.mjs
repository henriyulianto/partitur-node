...
async function generateIndexJson() {
  ...
  // Write index.json to both public and dist directories
  const publicOutputPath = path.join(publicOutputDir, 'index.json');
  const distOutputPath = path.join(distOutputDir, 'index.json');

  fs.writeFileSync(publicOutputPath, JSON.stringify(songs, null, 2));
  fs.writeFileSync(distOutputPath, JSON.stringify(songs, null, 2));

  console.log(`✅ Generated index.json with ${songs.length} songs`);
  console.log(`📍 Public output: ${publicOutputPath}`);
  console.log(`📍 Dist output: ${distOutputPath}`);
  ...
  return { publicOutputDir, distOutputDir };
}
...
async function generateSongDataFiles() {
  ...
  for (const dir of directories) {
    const song = processSongDirectory(dir, sourcePath);
    if (song) {
      // Create individual song directories in both public and dist
      const publicSongOutputDir = path.join(publicOutputDir, dir);
      const distSongOutputDir = path.join(distOutputDir, dir);

      if (!fs.existsSync(publicSongOutputDir)) {
        fs.mkdirSync(publicSongOutputDir, { recursive: true });
      }

      if (!fs.existsSync(distSongOutputDir)) {
        fs.mkdirSync(distSongOutputDir, { recursive: true });
      }

      // Write individual data.json to both directories
      const publicSongDataPath = path.join(publicSongOutputDir, 'data.json');
      const distSongDataPath = path.join(distSongOutputDir, 'data.json');

      fs.writeFileSync(publicSongDataPath, JSON.stringify(song, null, 2));
      fs.writeFileSync(distSongDataPath, JSON.stringify(song, null, 2));

      console.log(`📝 Generated: ${dir}/data.json (public & dist)`);
    }
  }
}
